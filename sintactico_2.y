...
expresion:
  termino
  {
    debug("Regla 35 : expresion = termino");
...
//	push(stackParentesis,TerminoP);
  ...
  {
    debug("\n Regla 39 ternimo es :termino * factor");
  /* 	if(TerminoP == FactorP){
  ...
  | termino {push(stackParentesis, TerminoP) ;} OP_DIV factor       {debug("Regla 40: termino dividido factor");}		{TerminoP = crearNodo("/", TerminoP, FactorP);}
;

factor:
	ID 							{procesarID(yylval.strVal);}					{debug("\nRegla 40");FactorP = crearHoja(yylval.strVal);}
	| TEXTO 				{procesarSTRING(yylval.strVal);}			{debug("\nRegla 41");FactorP = crearHoja(yylval.strVal);}
	| ENTERO    		{procesarINT(atoi(yylval.strVal));}		{debug("\nRegla 42" ); debug(yylval.strVal);FactorP = crearHoja(yylval.strVal);} //push(stackParentesis, FactorP);}
	| REAL  				{procesarFLOAT(atof(yylval.strVal));} {debug("\nRegla 43");FactorP = crearHoja(yylval.strVal);}
	| BOOLEAN
  | P_A expresion P_C
  {
    debug("\n Regla 44 ( exp )");
